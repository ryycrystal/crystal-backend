...
def test_pretrade_token_has_no_chart_points(db, clean):
    untraded = "0x" + "b1".rjust(40, "0")
    st = _new_state()
    created_at = int(time.time()) - 30
    _create(st, token=untraded, blk=100, ts=created_at)

    client = _api_client()
    overview = client.get(f"/token/{untraded}/60").json()
    chart = client.get(f"/chart/{untraded}/60").json()
