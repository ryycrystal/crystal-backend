...
async def main() -> None:
    ...
    if REINDEX:
        ...
        print(
            f"[IDX] Loaded {len(SEQUENCER._state.launchpad_tokens)} tokens, {len(SEQUENCER._state.v3_pools)} pools, "
            f"{len(getattr(SEQUENCER._state, 'addressToMarket', {}))} markets, {len(getattr(SEQUENCER._state, 'vaults', {}))} vaults",
            flush=True,
        )
    ...
    sampler_task = asyncio.create_task(vault_sampler(SEQUENCER._state))

    await asyncio.gather(stream_task, sampler_task)
