...
def main():
    ...
    print(f"[BAL] {len(rows):,} positions to check")
    ...
    for i in range(0, len(rows), args.batch):
        ...
        if (i // args.batch) % 20 == 0 and i:
            ...
            eta = (len(rows) - checked) / rate / 60 if rate else 0
            print(f"[BAL] {checked:,}/{len(rows):,} changed={changed:,} zeroed={zeroed:,} {rate:.0f}/s eta {eta:.0f}m")
